...
- **플로팅 버튼**: 읽기 모드에서 텍스트 선택 시 플로팅 버튼 표시, 바로 하이라이트 적용
- **컨텍스트 메뉴**: 편집 모드에서 우클릭(데스크톱) 또는 길게 누르기(모바일)로 하이라이트
...
1. 텍스트 선택
2. 나타나는 플로팅 버튼 클릭
3. 또는 단축키 사용 / 리본 아이콘 클릭

**편집 모드:**
1. 텍스트 선택
2. 우클릭(데스크톱) 또는 길게 누르기(모바일)
3. "Toggle highlight" 메뉴 선택
4. 또는 단축키 사용 / 리본 아이콘 클릭

**하이라이트 제거:**
- 이미 하이라이트된 텍스트에 다시 적용하면 제거
...
2. 플로팅 버튼 표시
3. 버튼 클릭 시:
   - 선택 위치 주변 문맥(context) 추출
   - 파일 내용 읽기
   - 3단계 매칭 전략으로 정확한 위치 찾기:
     * 문맥 기반 매칭 (1순위)
     * 라인 기반 매칭 (2순위)
...
**새 기능:**
- 읽기 모드 플로팅 버튼 추가 (텍스트 선택 시 자동 표시)
- 편집 모드 컨텍스트 메뉴 지원 (우클릭/길게 누르기)
- LRU 캐시 구현 (메모리 누수 방지)
- 멀티라인 선택 지원
